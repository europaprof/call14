...
from lift_alignment import PanelAlignment

RTSP_URL = os.environ['LIFT_RTSP_URL']
TEMPLATES_PATH = Path(os.getenv('LIFT_TEMPLATES', '/opt/call14/recognizer/templates-v4.json'))
SEGMENT_MODEL_PATH = Path(os.getenv('LIFT_SEGMENT_MODEL', '/opt/call14/recognizer/segment-v5.json'))
RED_SEGMENT_MODEL_PATH = Path(os.getenv('LIFT_RED_SEGMENT_MODEL', '/opt/call14/recognizer/segment-red-v6.json'))
STATE_PATH = Path(os.getenv('LIFT_STATE_FILE', '')) if os.getenv('LIFT_STATE_FILE') else None
...
HWACCEL = os.getenv('LIFT_HWACCEL', 'vaapi').strip().lower()
VAAPI_DEVICE = os.getenv('LIFT_VAAPI_DEVICE', '/dev/dri/renderD128')
LEFT_THRESHOLD = float(os.getenv('LIFT_LEFT_THRESHOLD', '60'))
...
RED_SEGMENT_POLYGONS = {
    'a': ((55,9),(65,8),(64,13),(54,13)),
    'b': ((64,13),(68,12),(67,24),(63,25)),
    'c': ((65,37),(69,37),(69,48),(65,48)),
    'd': ((58,50),(68,49),(66,54),(57,54)),
    'e': ((56,38),(60,37),(61,48),(57,49)),
    'f': ((56,15),(60,14),(60,24),(56,25)),
    'g': ((58,30),(67,29),(65,35),(57,35)),
}
...
def mqtt_publish(topic, payload, retain=True):
    ...
    try:
        result=subprocess.run(cmd, check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,timeout=3)
...
def publish_discovery():
    ...
    entities = {
        ('sensor','floor'): {'name':'Поверх','icon':'mdi:elevator','value_template':'{{ value_json.floor if value_json.floor is not none else "unknown" }}'},
        ('sensor','state'): {'name':'Стан','icon':'mdi:elevator-passenger','value_template':'{{ value_json.state }}'},
        ('sensor','direction'): {'name':'Напрямок','icon':'mdi:swap-vertical','value_template':'{{ value_json.direction }}'},
        ('sensor','display'): {'name':'Табло','icon':'mdi:monitor','value_template':'{{ value_json.display }}'},
        ('sensor','confidence'): {'name':'Впевненість','icon':'mdi:check-decagram','unit_of_measurement':'%','value_template':'{{ (value_json.confidence * 100) | round(0) }}'},
        ('sensor','floor_source'): {'name':'Джерело поверху','icon':'mdi:source-branch','value_template':'{{ value_json.floor_source }}'},
        ('sensor','last_seen'): {'name':'Останній кадр','device_class':'timestamp','value_template':'{{ value_json.last_seen }}'},
        ('binary_sensor','moving'): {'name':'Рухається','icon':'mdi:elevator-passenger','payload_on':'ON','payload_off':'OFF','value_template':'{{ "ON" if value_json.moving else "OFF" }}'},
        ('binary_sensor','fault'): {'name':'Несправність','device_class':'problem','payload_on':'ON','payload_off':'OFF','value_template':'{{ "ON" if value_json.fault else "OFF" }}'},
        ...
        ('binary_sensor','estimated'): {'name':'Поверх прогнозується','icon':'mdi:timer-sand','payload_on':'ON','payload_off':'OFF','value_template':'{{ "ON" if value_json.estimated else "OFF" }}'},
    }
...
class Recognizer:
    def __init__(self):
        ...
        self.red_segment_model=json.loads(RED_SEGMENT_MODEL_PATH.read_text()) if RED_SEGMENT_MODEL_PATH.exists() else None
        ...
        self.resync_count=0
        alignment_path=Path(os.getenv('LIFT_ALIGNMENT_MODEL','/opt/call14/recognizer/panel-alignment.json'))
        self.alignment=PanelAlignment(alignment_path) if alignment_path.exists() else None
        self.restore_state()

    def restore_state(self):
        """Keep a restart from turning cabin-light glare into a new position."""
        if not STATE_PATH:
            return
        try:
            saved=json.loads(STATE_PATH.read_text())
            floor=int(saved.get('confirmed_floor'))
            if floor in VALID_FLOORS:
                self.confirmed_floor=floor
                self.floor=floor
                self.last_change=saved.get('last_changed') or self.last_change
        except (OSError, ValueError, TypeError, json.JSONDecodeError):
            pass

    def save_state(self):
        if not STATE_PATH or self.confirmed_floor not in VALID_FLOORS:
            return
        try:
            STATE_PATH.parent.mkdir(parents=True,exist_ok=True)
            temporary=STATE_PATH.with_suffix(STATE_PATH.suffix+'.tmp')
            temporary.write_text(json.dumps({
                'confirmed_floor':self.confirmed_floor,
                'last_changed':self.last_change,
                'saved_at':now_iso(),
            },ensure_ascii=False))
            temporary.replace(STATE_PATH)
        except OSError as exc:
            print(f'Cannot persist recognizer state: {exc}',file=sys.stderr,flush=True)

    def classify_segment_model(self, display, model, channel='white'):
        if not model:
            return None, 9, 0.0, ''
        red,green,blue=display.split()
        signal=red if channel=='red' else ImageChops.darker(green,blue)
        pixels=signal.load()
        ...
        tens=model['tens']; tens_score=score(tens['mask'])
        floor=10+ones if tens_score>=tens['threshold'] else ones
        quality=min(evidence) if evidence else 0.0
        tolerated_errors=1 if channel=='red' else 0
        if floor not in VALID_FLOORS or errors>tolerated_errors:
            floor=None
        return floor,errors,quality,''.join(sorted(active))

    def classify_segments(self, display):
        primary=self.classify_segment_model(display,self.segment_model,'white')
    ...
    def classify_red_geometry(self, display):
        ...
        # relative red brightness, independent of that colour-phase change.
        from PIL import ImageDraw
        scores={}
        for name,polygon in RED_SEGMENT_POLYGONS.items():
            mask=Image.new('1',display.size)
            ImageDraw.Draw(mask).polygon(polygon,fill=1)
            values=sorted(r for (r,_,_),enabled in zip(display.getdata(),mask.getdata()) if enabled)
            upper=values[len(values)//2:]
            scores[name]=sum(upper)/max(1,len(upper))
        threshold=max(scores.values())*0.90
        active={name for name,value in scores.items() if value>=threshold}
        patterns={0:set('abcdef'),1:set('bc'),2:set('abdeg'),3:set('abcdg'),
                  4:set('bcfg'),5:set('acdfg'),6:set('acdefg'),7:set('abc'),
                  8:set('abcdefg'),9:set('abcdfg')}
        ranked=sorted((len(active^pattern),digit) for digit,pattern in patterns.items())
        errors,ones=ranked[0]
        ambiguity=ranked[1][0]-errors
        tens_on=False
        if self.red_segment_model:
            red=display.split()[0]; pixels=red.load()
            reference=sorted(pixels[x,y] for y in range(2,64) for x in range(37,71))
            scale=max(1.0,reference[int(len(reference)*0.95)])
            item=self.red_segment_model['tens']
            value=sum(pixels[x,y]/scale for x,y in item['mask'])/len(item['mask'])
            tens_on=value>=item['threshold']
        floor=10+ones if tens_on else ones
        if floor not in VALID_FLOORS:
            floor=None
        return floor,errors,ambiguity,''.join(sorted(active))
    ...
    def process(self, frame):
        if self.alignment:
            ...
            if self.alignment.quality<0.45:
                ...
                self.publish({'floor':None,'confirmed_floor':self.confirmed_floor,
                    ...
                    'last_seen':now_iso(),'source':'hybrid_segment_v6_aligned',
                    'alignment_quality':round(self.alignment.quality,3)},force=False)
                return
        ...
        left_core=arrow_feature(arrow_display,(24,44,29,62))
        left_background=arrow_feature(arrow_display,(34,44,39,62))
        right_core=arrow_feature(arrow_display,(74,15,79,34))
        right_background=arrow_feature(arrow_display,(84,15,89,34))
        left_on=left_core>55 and left_core-left_background>30
        right_on=right_core>35 and right_core-right_background>25
        if left_on and not right_on: raw_direction='up'
        elif right_on and not left_on: raw_direction='down'
        elif left_on and right_on: raw_direction='unknown'
        else: raw_direction='none'
        if raw_direction in ('up','down'):
            direction=raw_direction
            self.arrow_misses=0
        elif self.direction in ('up','down') and self.arrow_misses<8:
            self.arrow_misses+=1
        ...
        primary_segment,red_segment=self.classify_segments(arrow_display)
        segment_floor,segment_errors,segment_quality,segments=primary_segment
        red_floor,red_errors,red_quality,red_segments=red_segment
        geometry_floor,geometry_errors,geometry_margin,geometry_segments=self.classify_red_geometry(arrow_display)
        ...
        # Some camera/display timing combinations capture lit LED bars as pure
        # red instead of saturated white.  In that phase the legacy G/B signal
        # loses valid bars.  Use an independently trained red-channel decoder
        # only as a one-segment-tolerant fallback and still enforce physical
        # adjacency through allowed_now.
        red_reliable=(red_floor in allowed_now and red_errors<=1 and red_quality>=0.010)
        geometry_reliable=(geometry_floor in allowed_now and geometry_errors<=1 and geometry_margin>=1)
        # An incomplete red code must not overrule a complete physical code.
        # On descent, 'bcg' was corrected to 1 while the complete geometry
        # still read 'bcfg' (4), advancing to floor 1 before the display changed.
        if (red_errors>0 and geometry_errors==0 and geometry_margin>=1 and
                geometry_floor in VALID_FLOORS and red_floor!=geometry_floor):
            red_reliable=False
        geometry_jump=0
        if (direction in ('up','down') and self.confirmed_floor in VALID_FLOORS and
                geometry_floor in VALID_FLOORS and geometry_errors<=1 and geometry_margin>=1):
            old_index=VALID_FLOORS.index(self.confirmed_floor)
            new_index=VALID_FLOORS.index(geometry_floor)
            geometry_jump=new_index-old_index
            # A briefly missed digit must not lock the tracker forever. Strong
            # physical seven-segment geometry may catch up by at most three
            # valid floors, and only in the illuminated arrow direction.
            if ((direction=='up' and 0<=geometry_jump<=3) or
                    (direction=='down' and -3<=geometry_jump<=0)):
                geometry_reliable=True
        # Whole-image templates are useful when the multiplexed LED decoder misses
        # a phase, but weak matches are unsafe (the false live 7 -> 8 had
        ...
        if segment_reliable:
            ...
            floor=red_floor
            confidence=max(confidence,min(0.85,red_quality/0.10))
        ...
        decoder_reliable=segment_reliable or red_reliable or geometry_reliable
        # A template alone must never advance immediately when an arrow first
        # appears. That was the observed 1 -> 4 false jump. Give the car time
        ...
        resync_ok=resync_floor is not None
        if resync_ok and resync_floor!=self.confirmed_floor:
            if self.resync_candidate==resync_floor:
                self.resync_count+=1
            else:
                self.resync_candidate=resync_floor
                self.resync_count=1
            # Three seconds of stable, exactly matching physical segment codes.
            if self.resync_count>=max(6,round(3*FRAME_RATE)):
                floor=resync_floor
                reliable=True
                self.confirmed_floor=resync_floor
                self.floor=resync_floor
                self.estimated=False
                self.prediction_steps=0
                self.last_progress=now
                self.last_change=now_iso()
        ...
        if accepted_geometry:
            ...
            if direction in ('up','down'):
                confirmations=(4 if abs(geometry_jump)>1 else 2) if decoder_reliable else 8
            else:
                confirmations=5 if self.confirmed_floor is None else 4
        ...
        payload={'floor':self.floor,'state':state,'direction':direction,
                 ...
                 'distance':round(dist,3),'margin':round(margin,3),'activity':round(activity,1),'source':'hybrid_segment_v6_aligned',
                 'segment_floor':segment_floor,'segment_errors':segment_errors,
                 'segment_quality':round(segment_quality,3),'segments':segments,
                 'red_segment_floor':red_floor,'red_segment_errors':red_errors,
                 'red_segment_quality':round(red_quality,3),'red_segments':red_segments,
                 'geometry_floor':geometry_floor,'geometry_errors':geometry_errors,
                 'geometry_margin':geometry_margin,'geometry_segments':geometry_segments}
        if self.alignment:
            payload.update(alignment_offset=list(self.alignment.last),alignment_quality=round(self.alignment.quality,3))
        payload.update(left_contrast=round(left_core-left_background,1),right_contrast=round(right_core-right_background,1))
    ...
    def publish(self,payload,force=False):
        ...
        diagnostics=('last_seen','confidence','left_level','right_level','distance','margin','activity',
                     'all_floor','all_distance','all_margin','red_segment_floor',
                     'red_segment_errors','red_segment_quality','red_segments','segment_floor',
                     'segment_errors','segment_quality','segments','geometry_floor',
                     'geometry_errors','geometry_margin','geometry_segments','alignment_offset','alignment_quality',
        ...
        if force or material!=previous or now-self.last_publish>=30:
            if not mqtt_publish(f'{MQTT_PREFIX}/state',payload):
                return
...
def frame_stream():
    ...
    cmd += ['-rtsp_transport','tcp','-timeout','10000000','-i',RTSP_URL,
         '-vf',','.join(filters),
         '-q:v','3','-f','image2pipe','-vcodec','mjpeg','pipe:1']
    # Drain errors directly to the journal; an unread PIPE can block FFmpeg.
    process=subprocess.Popen(cmd,stdout=subprocess.PIPE,stderr=None,bufsize=0)
    data=bytearray()
    last_complete=time.monotonic()
    try:
        while RUNNING:
            if time.monotonic()-last_complete>15:
                raise TimeoutError('No complete RTSP frame for 15 seconds')
            ...
            if len(data)>2*1024*1024:
                raise ValueError('RTSP JPEG buffer exceeds 2 MiB')
            while True:
                ...
                try: frame=Image.open(io.BytesIO(jpg)).convert('RGB')
...
def main():
    ...
    while RUNNING:
        ...
        recognizer.candidate=None;recognizer.candidate_count=0
        recognizer.resync_candidate=None;recognizer.resync_count=0
